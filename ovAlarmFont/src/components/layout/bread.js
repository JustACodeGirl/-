...
function Bread ({ location }) {
  ...
  const breads = pathNames.map((item, key) => {
    ...
    return (
      <Breadcrumb.Item style={{ color: '#fefefe' }} key={key} {...((pathNames.length - 1 === key) || !pathSet[item].clickable) ? '' : { href: `#${pathSet[item].path}` }}>
        {pathSet[item].icon
  ...
  return (
    <div className={styles.bread}>
      <Breadcrumb>
        <Breadcrumb.Item style={{color: '#f0f0f0'}} href="/"><Icon type="home" />
          <span>主页</span>
        </Breadcrumb.Item>
        {breads}
      </Breadcrumb>
